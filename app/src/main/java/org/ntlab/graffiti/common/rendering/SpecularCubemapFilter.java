...
public class SpecularCubemapFilter implements Closeable {
   ...
   */
  public SpecularCubemapFilter(Context context, int resolution, int numberOfImportanceSamples)
      throws IOException {
    ...
    try {
      radianceCubemap =
          new Texture(GLES30.GL_TEXTURE_CUBE_MAP, GLES30.GL_CLAMP_TO_EDGE);
      ldCubemap =
          new Texture(GLES30.GL_TEXTURE_CUBE_MAP, GLES30.GL_CLAMP_TO_EDGE);

      ChunkIterable chunks = new ChunkIterable(getMaxColorAttachments());
      initializeLdCubemap();
      shaders = createShaders(context, chunks);
...
//              GLES30.GL_TRIANGLE_FAN,
  ...
  private Shader[] createShaders(Context context, ChunkIterable chunks) throws IOException {
    ...
    for (Chunk chunk : chunks) {
      ...
      shaders[chunk.chunkIndex] =
          Shader.createFromAssets(
                  context, "shaders/cubemap_filter.vert", "shaders/cubemap_filter.frag", defines)
