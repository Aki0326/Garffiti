 ...
 */
public class ColoringBattleManager {
    private static final String TAG = ColoringBattleActivity.class.getSimpleName() + "." + ColoringBattleManager.class.getSimpleName();
    ...
    interface CloudAnchorHostListener {
        ...
        void onCloudHostTaskComplete(Anchor anchor, PointF coordinate);
    }
    ...
    interface CloudAnchorResolveListener {
        ...
        /** This method is invoked when the results of a Cloud Anchor operation are available. */
        void onCloudResolveTaskComplete(Anchor anchor, PointF coordinate);
    ...
    private final HashMap<Anchor, PointF> pendingHostCoordinates = new HashMap<>();
    private final HashMap<Anchor, CloudAnchorResolveListener> pendingResolveAnchors = new HashMap<>();
    private final HashMap<Anchor, PointF> pendingResolveCoordinates = new HashMap<>();
     ...
     * available.
     */
//    synchronized void resolveCloudAnchor(String anchorId, CloudAnchorResolveListener listener, long startTimeMillis, PointF coordinate) {
//        Preconditions.checkNotNull(session, "The session cannot be null.");
...
//        pendingResolveCoordinates.put(newAnchor, coordinate);
//    }
    ...
    synchronized void onUpdate() {
        Preconditions.checkNotNull(session, "The session cannot be null.");
        Iterator<Map.Entry<Anchor, CloudAnchorHostListener>> hostIter = pendingHostAnchors.entrySet().iterator();
...
//                }
        ...
        Iterator<Map.Entry<Anchor, CloudAnchorResolveListener>> resolveIter = pendingResolveAnchors.entrySet().iterator();
        while (resolveIter.hasNext()) {
            ...
            if (isReturnableState(anchor.getCloudAnchorState())) {
                PointF coordinate = pendingResolveCoordinates.get(anchor);
                listener.onCloudResolveTaskComplete(anchor, coordinate);
                resolveIter.remove();
            }
